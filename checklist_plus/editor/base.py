...
from checklist_plus.multilingual import get_language_code, multilingual_params
from checklist_plus.viewer.template_editor import TemplateEditor
...
class Editor:
    def __init__(self, language='english', model_name=None, use_llm=False, llm_client=None, openai_api_key=None, **kwargs):
        self.lexicons = {}
        self.data = {}
        self.tg_params = {
            'language': language,
        ...
        # Store LLM parameters for potential use
    ...
    def _load_lexicons(self, language):
        # Get the project root directory (one level up from editor folder)
        project_root = os.path.dirname(os.path.dirname(__file__))
        folder = os.path.abspath(os.path.join(project_root, "data", 'lexicons'))
        for f in os.listdir(folder):
            self.lexicons.update(json.load(open(os.path.join(folder, f))))
        self.data['names'] = json.load(open(os.path.join(project_root, 'data', 'names.json')))
        ...
        wikidata = pickle.load(open(os.path.join(project_root, 'data', 'wikidata.pkl'), 'rb'))
    ...
    def __getattr__(self, attr):
        if attr == 'tg':
            if self.use_llm:
                ...
                self.tg = LLMTextGenerator(
                    llm_client=self.llm_client,
                    openai_api_key=self.openai_api_key,
                    model_name=self.tg_params.get('model_name', 'gpt-3.5-turbo')
                )
            else:
                from checklist_plus.text_generation import TextGenerator
    ...
    def _wordnet_stuff(self, templates, word, type, threshold=5, depth=3, pos=None, **kwargs):
        ...
        return [x[0][0] for x in fn(texts, word, threshold=threshold, pos=pos, depth=depth, type=type)]
    ...
    def paraphrase_llm(self, templates, n_paraphrases=5, context=None, style=None,
                   length_preference=None, preserve_meaning=True,
                   temperature=0.7, unroll=False, **kwargs):
        """Generate paraphrases of templates using LLM
        ...
        templates : str, list, tuple, or dict
            On leaves: templates with {tags}, which will be substituted for mapping in **kwargs
        n_paraphrases : int
            Number of paraphrases to generate per template
        ...
        unroll : bool
            If True, returns flat list of strings regardless of template structure
        **kwargs : type
            Template fill-in parameters, same as template() method

        Returns
        -------
        MunchWithAdd or list
            If unroll=True: list of paraphrased strings
            Otherwise: MunchWithAdd with paraphrases in ret.data, preserving template structure

        Raises
        ------
        Exception
            If LLM is not enabled (use_llm=False)
        """
        if not self.use_llm:
            raise Exception("Paraphrase feature requires LLM text generator. Initialize Editor with use_llm=True")

        # Check if text generator has paraphrase method
        if not hasattr(self.tg, 'paraphrase'):
            raise Exception("Text generator does not support paraphrasing. Ensure you're using LLMTextGenerator")

        # First, fill in the templates to get concrete sentences
        filled_templates = self.template(templates, unroll=True, **kwargs)
        sentences = filled_templates.data

        if not sentences:
            ret = MunchWithAdd()
            ret.data = []
            return ret.data if unroll else ret

        # Use the text generator's paraphrase method
        try:
            paraphrases = self.tg.paraphrase(
                texts=sentences,
            ...
            paraphrases = sentences * n_paraphrases
        ...
        if unroll:
            ...
            ret = MunchWithAdd()
            ret.data = paraphrases
            if hasattr(filled_templates, 'meta'):
                # Repeat meta information for each paraphrase
                ret.meta = []
                for meta_item in filled_templates.meta:
                    ret.meta.extend([meta_item] * n_paraphrases)
            return ret

    def detect_entities_llm(self, templates, entity_type, unroll=False, **kwargs):
        """Detect entities in templates using LLM
        ...
        unroll : bool
            If True, returns flat list of detection results regardless of template structure
        **kwargs : type
            Template fill-in parameters, same as template() method
        ...
        -------
        MunchWithAdd or list
            If unroll=True: list of detection result dictionaries
            Otherwise: MunchWithAdd with detection results in ret.data, preserving template structure
        ...
        """
        if not self.use_llm:
            raise Exception("Entity detection feature requires LLM text generator. Initialize Editor with use_llm=True")

        # Check if text generator has detect_entities method
        if not hasattr(self.tg, 'detect_entities'):
            raise Exception("Text generator does not support entity detection. Ensure you're using LLMTextGenerator")
        ...
        # Use the text generator's detect_entities method
        try:
            detection_results = self.tg.detect_entities(
                texts=sentences,
                entity_type=entity_type
            )
        except Exception as e:
            logger.error(f"Entity detection failed: {e}", exc_info=True)
            # Fallback: return empty detection results
            detection_results = [
                {
                    "text": sentence,
        ...
        if unroll:
            ...
            # Preserve template structure
            ret = MunchWithAdd()
            ret.data = detection_results
            if hasattr(filled_templates, 'meta'):
                ret.meta = filled_templates.meta
            return ret

    def detect_and_mask_entities_llm(self, templates, entity_type, mask_token="{mask}",
                                   case_sensitive=False, whole_words_only=True,
                                   unroll=False, **kwargs):
        """Detect and mask entities in templates using LLM
        ...
        templates : str, list, tuple, or dict
            On leaves: templates with {tags}, which will be substituted for mapping in **kwargs
        entity_type : str
            Type of entity to detect and mask (e.g., "brand names", "person names", "animals")
        mask_token : str
        ...
        unroll : bool
            If True, returns flat list of results regardless of template structure
        **kwargs : type
            Template fill-in parameters, same as template() method
        ...
        -------
        MunchWithAdd or list
            If unroll=True: list of detection and masking result dictionaries
            Otherwise: MunchWithAdd with results in ret.data, preserving template structure
        ...
        """
        if not self.use_llm:
            raise Exception("Entity detection and masking feature requires LLM text generator. Initialize Editor with use_llm=True")

        # Check if text generator has detect_and_mask_entities method
        if not hasattr(self.tg, 'detect_and_mask_entities'):
            raise Exception("Text generator does not support entity detection and masking. Ensure you're using LLMTextGenerator")
        ...
        # Use the text generator's detect_and_mask_entities method
        try:
            results = self.tg.detect_and_mask_entities(
                texts=sentences,
                entity_type=entity_type,
            ...
            logger.error(f"Entity detection and masking failed: {e}", exc_info=True)
            # Fallback: return original sentences with no masking
            results = [
                {
